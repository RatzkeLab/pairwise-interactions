...
sys.path.insert(0, str(HERE.parents[1]))                       # config.py (analysis root)

import pandas as pd
from config import CFG, out_dir, LAYOUT_CSV_FULL, OD_FULL, OD_PREP
...
gml = gm.GenomicMLConfig(exp_cfg=CFG, out_dir=out_dir("06_ml_analysis/genomic_ml"))
cfg = gp.PlateMLConfig(
    gml=gml,
    layout_csv=LAYOUT_CSV_FULL,
    od_full_dir=OD_FULL,     # 61-channel scans of the 30 destination plates
    mapping_validation_csv=CFG.mapping_validation_out_dir / "05_combined_sample_summary.csv",
    out_dir=out_dir("06_ml_analysis/genomic_ml_plate"),
    source_od_dir=OD_PREP,        # the pre-experiment 384-collection reads
)
